...
import { isAvailable, availableAt } from "./ratelimit.js";
...
function firstAvailableFrom(pool, start, lane, now) {
  const n = pool.accounts.length;
  for (let step = 0; step < n; step++) {
    const i = (start + step) % n;
    if (isAvailable(pool.accounts[i], lane, now)) return i;
  }
  return -1;
}
...
// returns the soonest-free index even when it is not yet usable.
export function selectIndex(pool, lane, now, strategy) {
  const n = pool.accounts.length;
  if (n === 0) return -1;
  ...
  if (strat === "round-robin") {
    const i = firstAvailableFrom(pool, (cursor + 1) % n, lane, now);
    if (i >= 0) setLaneCursor(pool, lane, i);
    return i;
  }

  if (cursor >= 0 && cursor < n && isAvailable(pool.accounts[cursor], lane, now)) return cursor;

  const i = firstAvailableFrom(pool, cursor, lane, now);
